...
class ExecuteStage(Module):
    ...
    def build(self, id_ex_valid, id_ex_pc, id_ex_rs1_idx, id_ex_rs2_idx, id_ex_immediate, id_ex_control, ex_mem_pc, ex_mem_control, ex_mem_valid, ex_mem_result, ex_mem_data, ex_mem_target_pc, ex_mem_pc_change, reg_file, memory_stage):
        ...
        execute_signals = concat(
            ex_mem_valid[0].select(id_ex_valid[0].select(rs2_data, UInt(XLEN)(0)), ex_mem_data[0]),                                      # rs2_data (store数据)
            ex_mem_valid[0].select(id_ex_valid[0].select(alu_result, UInt(XLEN)(0)), ex_mem_result[0]),                                # ALU结果
...
class MemoryStage(Module):
    ...
    def build(self, ex_mem_valid, ex_mem_result, ex_mem_pc, ex_mem_data, ex_mem_control, mem_wb_control, mem_wb_valid, mem_wb_mem_data, mem_wb_ex_result, writeback_stage, data_sram):
        ...
        memory_signals = concat(
            mem_wb_valid[0].select(ex_mem_valid[0].select(ex_mem_result[0], UInt(XLEN)(0)), mem_wb_ex_result[0]),                        # EX结果
            mem_wb_valid[0].select(ex_mem_valid[0].select(control_in.bitcast(Bits(CONTROL_LEN)), Bits(CONTROL_LEN)(0)), mem_wb_control[0].bitcast(Bits(CONTROL_LEN))),  # 控制信号
        )
